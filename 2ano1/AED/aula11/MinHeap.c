...
void MinHeapRemoveMin(MinHeap *ph) {
  ...
  while (1) {
    ...
    if (!(min < ph->size))
      break; // if no second child, stop looking

    unsigned int second_child = _child(n, 2);

    // if second child is smaller, choose it
    if (ph->compare(ph->array[second_child], ph->array[min]) < 0) {
      min = second_child;
    }

    // if smallest child is not smaller than last, stop looking
    if (!(ph->compare(ph->array[min], ph->array[ph->size]) < 0))
      break;
...
int MinHeapCheck(MinHeap *ph) {
  ...
  for (unsigned int n = 1; n < ph->size; n++) {
    int p = _parent(n);
    if (ph->compare(ph->array[p], ph->array[n]) > 0)
      return 0;
  }
  return 1;
}
