...
static int _treeGetHeightREC(const struct _BSTreeNode *root) {
  ...
  const int child_height =
      MAX(_treeGetHeightREC(root->left), _treeGetHeightREC(root->right));
...
void *BSTreeGetMin(const BSTree *header) {
  ...
  const struct _BSTreeNode *current = header->root;
...
void *BSTreeSearch(const BSTree *header, const void *item) {
  ...
  while (current != NULL) {
    const int res = header->compare(item, current->item);
    if (res == 0) {
      return current->item;
    } else if (res > 0) {
